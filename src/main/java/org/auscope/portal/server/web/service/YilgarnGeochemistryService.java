...
/**
 * Service class providing methods for interacting with a WFS that implements the Yilgarn Laterite Geochemistry information model
 *
 * @author Josh Vote
 *
 */
...
public class YilgarnGeochemistryService extends BaseWFSService {
     ...
     * @param methodMaker
     *            Will be used to generate HTTP methods
     */
    @Autowired
    ...
    /**
     * Evaluates the specified xPath and returns the result as a string. If null is evaluated the empty string will instead be returned.
     * 
     * @param xPath
     *            The XPath to be evaluated
     * @param node
     *            The node that xPath should evaluated on
     * @return
     */
    ...
    /**
     * Given a URL of a WFS and a ID for a sa:LocatedSpecimen, lookup the located specimen feature and return a simple parsed response.
     *
     * Will return null if locSpecimenId returns no matching features
     *
     * @param serviceUrl
     *            A URL referencing a Yilgarn Laterite Geochemistry WFS
     * @param locSpecimenId
     *            a gml id for serviceUrl that references a sa:LocatedSpecimen
     * @return
     */
    public YilgarnLocatedSpecimenRecord getLocatedSpecimens(String serviceUrl, String locSpecimenId)
        ...
        try {
            ...
            XPathExpression xPathLocSpecs = DOMUtil.compileXPathExpr(String.format(
                    "/wfs:FeatureCollection/gml:featureMembers/%1$s | /wfs:FeatureCollection/gml:featureMember/%1$s",
                    LOCATED_SPECIMEN_TYPENAME), nc);
            ...
            for (int i = 0; i < observationNodes.getLength(); i++) {
                ...
                String dateAndTime = xPathEvalString("om:samplingTime/gml:TimeInstant/gml:timePosition",
                        observationNode, nc);
                String date = dateAndTime.split(" ")[0];
                String observedMineralName = xPathEvalString("om:procedure/omx:ObservationProcess/@gml:id",
                        observationNode, nc);
                String preparationDetails = xPathEvalString("om:procedure/omx:ObservationProcess/gml:description",
                        observationNode, nc);
                String labDetails = xPathEvalString("om:procedure/omx:ObservationProcess/sml:contact/@xlink:title",
                        observationNode, nc);
                String analyticalMethod = xPathEvalString("om:procedure/omx:ObservationProcess/omx:method",
                        observationNode, nc);
                ...
                observations.add(new YilgarnObservationRecord(serviceName, date, observedMineralName,
                        preparationDetails,
                        labDetails, analyticalMethod, observedProperty, analyteName, analyteValue, uom));
            }

            return new YilgarnLocatedSpecimenRecord(observations.toArray(new YilgarnObservationRecord[observations
                    .size()]), materialClass);
        } catch (Exception ex) {

/**
 * AN extension to a normal filter that treats the spatial attribute as a 'special case'
 * to allow the referencing of a complex BBox object WHILST also allowing serialisation into JSON
 *
 * events :
 ...
 */
Ext.define('portal.layer.filterer.SpatialFilterer', {
    extend: 'portal.layer.filterer.Filterer',
    ...
    getParameters : function() {
        ...
        if (this.bbox) {
            params[portal.layer.filterer.SpatialFilterer.BBOX_FIELD] = Ext.util.JSON.encode(this.bbox);
        }
    ...
    getParameter : function(key) {
        if (key === portal.layer.filterer.SpatialFilterer.BBOX_FIELD) {
            if (this.bbox) {
                return Ext.util.JSON.encode(this.getSpatialParam());
            } else {
                return undefined;
            }
        } else {

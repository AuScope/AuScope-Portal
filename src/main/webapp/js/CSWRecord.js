...
/**
 * Given a KnownFeatureTypeStore this function will return an array of KnownFeatureTypes that 
 * this CSWRecord is representing
 */
CSWRecord.prototype.getLinkedKnownFeatureTypes = function(knownFeatureTypeStore) {
	var kfts = [];
	var onlineResources = this.getOnlineResources();
	for (var i = 0; i < onlineResources.length; i++) {
		var kft = knownFeatureTypeStore.getKnownFeatureTypeById(onlineResources[i].name);
		if (kft) {
			kfts.push(kft);
		}
	}

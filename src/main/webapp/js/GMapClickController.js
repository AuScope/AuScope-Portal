...
var gMapClickController = function(map, overlay, latlng, activeLayersStore) {
	...
	if (parentActiveLayerRecord) {
		...
		var cswRecords = parentActiveLayerRecord.getCSWRecordsWithType('WFS');
	    if (cswRecords.length != 0) {
	    	...
	    	if (overlay instanceof GMarker) {
	            if (wfsTypeName === "gsml:Borehole") {
	                new NvclInfoWindow(map,overlay, wfsUrl).show();
	            }
	            else if (wfsTypeName == "ngcp:GnssStation") {
	                new GeodesyMarker(wfsUrl, "geodesy:station_observations", overlay.title, overlay, overlay.description).getMarkerClickedFn()();
	            }

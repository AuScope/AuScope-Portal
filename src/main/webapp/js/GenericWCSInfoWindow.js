    ...
    var startLoading = function(map, location, opts) {
        Ext.Ajax.request({
            ...
            success	: function(response, options) {
                ...
                //Generate an error / success fragment to display to the user
                if (responseObj.success && responseObj.data && responseObj.data.length > 0) {
                    var record = responseObj.data[0]; //We only ever check the first record because only 1 should be returned

Rickshaw.namespace('Rickshaw.Graph.RangeSlider.MultiPreview');

Rickshaw.Graph.RangeSlider.MultiPreview = Rickshaw.Class.create({

	initialize: function(args) {

		if (!args.element) throw "Rickshaw.Graph.RangeSlider.MultiPreview needs a reference to an element";
		if (!args.graph && !args.graphs) throw "Rickshaw.Graph.RangeSlider.MultiPreview needs a reference to an graph or an array of graphs";
	...
	configure: function(args) {
		...
		if ('width' in args || 'height' in args) {
			...
			this.previews.forEach(function(preview) {
				...
				if (this.svg) {
					var svgHeight = this.previewHeight + this.config.frameTopThickness * 2;
	...
	render: function() {
		...
		var buildGraph = function(parent, index) {
			...
			// Build small graphs in the preview window

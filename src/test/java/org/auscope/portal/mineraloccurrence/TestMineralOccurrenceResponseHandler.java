...
public class TestMineralOccurrenceResponseHandler extends PortalTestClass {
    ...
    public void testHandleMineResponse() throws Exception {
        String mineGetFeatureResponse = ResourceUtil
                .loadResourceAsString("org/auscope/portal/erml/mine/mineGetFeatureResponse.xml");
        ...
        Assert.assertEquals("The first mine is WOOLDRIDGE CREEK WORKINGS", "WOOLDRIDGE CREEK WORKINGS",
                ((Mine) mines.toArray()[0]).getMineNamePreffered());
        Assert.assertEquals("The second mine is HALL MAGNESITE MINE", "HALL MAGNESITE MINE",
                ((Mine) mines.toArray()[1]).getMineNamePreffered());
    }

    @Test
    public void testHandleCommodityResponse() throws Exception {
        String commodityGetFeatureResponse = ResourceUtil
                .loadResourceAsString("org/auscope/portal/erml/commodity/commodityGetFeatureResponse.xml");

        Collection<Commodity> commodities = mineralOccurrencesResponseHandler
                .getCommodities(commodityGetFeatureResponse);

        Assert.assertEquals("There are 2 commodities", 2, commodities.size());
        Assert.assertEquals("The first one's name is Gold", "Gold",
                ((Commodity) commodities.toArray()[0]).getCommodityName());
        Assert.assertEquals(
                "The second one's MineralOccurrence source is urn:cgi:feature:GSV:MineralOccurrence:361170",
                "urn:cgi:feature:GSV:MineralOccurrence:361170", ((Commodity) commodities.toArray()[1]).getSource());
    }
    ...
    public void testGetNumberOfFeaturesZero() throws Exception {
        String getFeatureResponse = ResourceUtil
                .loadResourceAsString("org/auscope/portal/erml/minocc/mineralOccurrenceNoFeaturesResponse.xml");
